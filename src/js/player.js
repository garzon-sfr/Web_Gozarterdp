...
class VintageRadioPlayer {
  constructor() {
    ...
    this.FIXED_STREAM_URL =
      "https://panel.gozarterdp.com/listen/gozarte_radio/radio.mp3";
  ...
  setupAudioElement() {
    ...
    this.elements.audioElement.volume =
      parseInt(this.elements.volumeSlider.value, 10) / 100;
  ...
  async updateMediaSessionMetadata(metadata) {
    ...
    const artworkUrl =
      metadata.artUrl ||
      this.elements.songArtworkImg.src ||
      this.elements.artworkMain.querySelector("img")?.src ||
      "";
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      ...
      artwork: [{ src: artworkSrc, sizes: "512x512", type: "image/webp" }],
    });
    navigator.mediaSession.playbackState = this.elements.audioElement.paused
      ? "paused"
      : "playing";
  }
  ...
  updateVolumeIcon(isMuted) {
    if (isMuted) {
      this.elements.volumeButton.innerHTML =
  ...
  fadeVolume(targetVolume, duration = 500) {
    ...
    const fadeInterval = setInterval(() => {
      currentStep++;
      this.elements.audioElement.volume = Math.max(
        0,
  ...
  setupNetworkSensors() {
    ...
    audio.addEventListener("progress", () => {
      ...
      if (buffered.length > 0) {
        console.log(
          `📶 Buffer: ${Math.round(buffered.end(0) - buffered.start(0))}s descargados`,
        );
      }
    });
    ...
    audio.addEventListener("stalled", () => {
      console.error(
        "❌ Error crítico: El servidor de la radio dejó de enviar datos (Stalled)",
      );
    });
  }
  ...
  addNotificationButton() {
    setTimeout(() => {
      ...
      if (isAlreadySubscribed && Notification.permission === "granted") {
        notifBtn.innerHTML =
          '<i class="fas fa-check"></i> Notificaciones activas';
